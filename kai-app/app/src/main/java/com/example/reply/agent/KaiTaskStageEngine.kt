...
object KaiTaskStageEngine {
    ...
    fun classifyGoalMode(prompt: String): GoalMode {
        ...
        val followUpSignals = listOf(
            " then ", " and ", "ثم", "بعد",
            "message", "messages", "chat", "dm", "conversation",
            "note", "write", "type", "send", "reply", "play", "watch",
    ...
    fun evaluate(
        ...
        val appEntryComplete = when (openAppOutcome) {
            ...
            KaiOpenAppOutcome.USABLE_INTERMEDIATE_IN_TARGET_APP -> true
            else -> appHint.isNotBlank() && usableForApp && currentState.likelyMatchesAppHint(appHint)
        }

        if (!appEntryComplete) {
            return StageSnapshot(goalMode, Stage.APP_ENTRY, false, false, true, "open_app", "app_entry_not_confirmed")
        }
        if (goalMode == GoalMode.OPEN_ONLY) {
            return StageSnapshot(goalMode, Stage.SUCCESS, true, true, false, "none", "open_only_goal_satisfied")
        }
        ...
        if (finalGoalComplete) {
            return StageSnapshot(goalMode, Stage.SUCCESS, true, true, false, "none", "stage_goal_satisfied")
        }
    ...
    fun buildContinuationStep(
        ...
        return when (stageSnapshot.nextSemanticAction) {
            ...
            "open_messages" -> {
                val entry = currentState.findMessagesEntry()
                val target = entry?.contentDescription?.takeIf { it.isNotBlank() }
                    ?: entry?.text?.takeIf { it.isNotBlank() }
                    ?: if (currentState.packageName.contains("whatsapp", true)) "chats" else "messages"
            ...
            "open_target_conversation" -> {
                val query = extractConversationQuery(userPrompt)
                val candidate = currentState.findConversationCandidate(query)
                val target = candidate?.text?.takeIf { it.isNotBlank() }
                    ?: candidate?.contentDescription?.takeIf { it.isNotBlank() }
                    ?: query
            ...
            "open_note_editor" -> {
                val create = currentState.findCreateAction()
                val target = create?.contentDescription?.takeIf { it.isNotBlank() }
                    ?: create?.text?.takeIf { it.isNotBlank() }
                    ?: "new note"
