...
- `KaiAIClient.kt` L228: `val key = BuildConfig.OPENAI_API_KEY.trim()`
- `KaiAIClient.kt` L285: same (streaming path)
- `KaiVoice.kt`: `val key = BuildConfig.OPENAI_API_KEY.trim()`

Empty-key guard in `KaiAIClient.ask()` (L229–231):
```kotlin
if (key.isBlank()) {
    return "OpenAI API key is missing. Put it in gradle.properties as OPENAI_API_KEY=..."
}
```
Empty-key guard in `KaiAIClient.askStream()` (L286–289):
```kotlin
if (key.isBlank()) {
...
Environment variable (Codespaces secret)
  ...
  → KaiAIClient.ask() / askStream() / KaiVoice.kt at runtime
```
...
- **Imported as**: `import com.example.reply.ai.KaiModelRouter` in `KaiAIClient.kt` and `KaiVoice.kt`
...
| KaiVoice TTS uses gpt-4o-audio-preview | LOW | MODEL_TTS = `"gpt-4o-audio-preview"` — this model may not be available on all OpenAI accounts or tiers. If unavailable, TTS calls will fail with OpenAI error 404. The KaiAIClient returns the error as a string, not an exception — non-crashing but non-functional. |
...
| Variable rename detection | SAFE | All 3 BuildConfig field names (`OPENAI_API_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`) match exactly between `build.gradle.kts` (definition) and all runtime consumers (KaiAIClient.kt, KaiVoice.kt, SupabaseClientProvider.kt). No mismatch found. |
